#pragma once

// Name: Fortnite, Version: OT-6_5


/*!!DEFINE!!*/

/*!!HELPER_DEF!!*/

/*!!HELPER_INC!!*/

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
//---------------------------------------------------------------------------
// Parameters
//---------------------------------------------------------------------------

// Function GA_Floor_Health.GA_Floor_Health_C.K2_ActivateAbilityFromEvent
struct UGA_Floor_Health_C_K2_ActivateAbilityFromEvent_Params
{
	struct FGameplayEventData                          EventData;                                                 // (ConstParm, Parm, OutParm, ReferenceParm)
};

// Function GA_Floor_Health.GA_Floor_Health_C.K2_ActivateAbility
struct UGA_Floor_Health_C_K2_ActivateAbility_Params
{
};

// Function GA_Floor_Health.GA_Floor_Health_C.ExecuteUbergraph_GA_Floor_Health
struct UGA_Floor_Health_C_ExecuteUbergraph_GA_Floor_Health_Params
{
	int                                                EntryPoint;                                                // (Parm, ZeroConstructor, IsPlainOldData, NoDestructor)
};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
