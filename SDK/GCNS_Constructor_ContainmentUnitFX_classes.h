#pragma once

// Name: Fortnite, Version: OT-6_5


/*!!DEFINE!!*/

/*!!HELPER_DEF!!*/

/*!!HELPER_INC!!*/

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
//---------------------------------------------------------------------------
// Classes
//---------------------------------------------------------------------------

// BlueprintGeneratedClass GCNS_Constructor_ContainmentUnitFX.GCNS_Constructor_ContainmentUnitFX_C
// 0x0000 (FullSize[0x0044] - InheritedSize[0x0044])
class UGCNS_Constructor_ContainmentUnitFX_C : public UFortGameplayCueNotify_Simple
{
public:


	static UClass* StaticClass()
	{
		static UClass* ptr = UObject::FindClass("BlueprintGeneratedClass GCNS_Constructor_ContainmentUnitFX.GCNS_Constructor_ContainmentUnitFX_C");
		return ptr;
	}



};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
